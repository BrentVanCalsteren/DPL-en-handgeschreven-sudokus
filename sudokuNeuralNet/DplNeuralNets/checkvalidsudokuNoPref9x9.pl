nn(mnist_net,[X],Y,[1,2,3,4,5,6,7,8,9]) :: convertFoto(X,Y).
...
checkValidSudoku(In):-
    ...
    hasAllUniqueNumberss(L1),
    hasAllUniqueNumberss(L2),
    hasAllUniqueNumberss(L3),
    checkContraintss(Vars-List,Links,Li).
    %-----------------------------------------------
...
hasAllUniqueNumbers([]).
hasAllUniqueNumbers([N|Rest]):-
   nomemberCostum(N,Rest),hasAllUniqueNumbers(Rest).
...
linkWithVar([El|L],[A|Links]-[A|Vars]):- El == empty, linkWithVar(L,Links-Vars).
linkWithVar([El|L],[A|Links]-[A|Vars]):- El == "empty",linkWithVar(L,Links-Vars).
...
checkContraintss([]-[],[],_).
checkContraintss([E|El]-[Li|Lis],[Link|Links],List):-
    checkContraints(E-Li,Link,List),
    checkContraintss(El-Lis,Links,List).

checkContraints([]-[],[],_).
checkContraints([E|El]-[Li|List],[Link|Links],L):-
    checkContraint(E,Link,Li,L),checkContraints(El-List,Links,L).

checkContraint(E,Link,L,List):- var(Link),member(E,List), nomemberCostum(E,L).
checkContraint(E,Link,L,List):-
    compound(Link), member(E,List), nomemberCostum(E,L), convertFoto(Link,E).
