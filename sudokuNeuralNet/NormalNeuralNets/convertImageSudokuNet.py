...
class convertImageSudokuNet(neuralNet):
    def __init__(self, data_set, lr, epoch):
        super().__init__(epoch,data_set)
        self.model = torchNet.convert_sudoku_image_to_number(self.sqlength**2, 28)
    ...
    def get_loss(self, target):
        ...
        self.last_targetValue = torch.tensor(target, dtype=torch.float32).view(self.sqlength**2)
